...
export const parseStringDefinition = (
  stringDefinition: string
): ParserDefinition => {
  if (stringDefinition === "string" || stringDefinition === "number") {
    return { type: stringDefinition };
  }
  const [, , parserStringDefinitionForItems = "number", , length] =
    stringDefinition.match(/^\s*(<([^>]*)>)?(\[(\w+)\])\s*$/) || [];
  if (length) {
    return {
      type: "array",
      length,
      items: parseStringDefinition(parserStringDefinitionForItems)
    };
  }
  const [, propertiesStringDefinitions] =
    stringDefinition.match(/^\s*\{(.*)\}\s*$/s) || [];
  if (propertiesStringDefinitions) {
    const propertyParserDefinitions: PropertyParserDefinition[] = parsePropertiesStringDefinitions(
      propertiesStringDefinitions
    );
    return {
      type: "object",
      properties: propertyParserDefinitions
    };
  }
  throw new RangeError(`cannot parse '${stringDefinition}'`);
};

const parsePropertiesStringDefinitions = (
  stringDef: string
): PropertyParserDefinition[] => {
  return Array.from(stringDef.matchAll(/(\w+)(<([^>]*)>)?(\[(\w+)\])?/g)).map(
    ([
      ,
      propertyName,
      ,
      parserStringDefinitionForProperty = "number",
      isArray,
      length
    ]) => {
      const parserDefinition = parseStringDefinition(
        parserStringDefinitionForProperty
      );
      return isArray
        ? {
            [propertyName]: { type: "array", length, items: parserDefinition }
          }
        : { [propertyName]: parserDefinition };
    }
  );
};
